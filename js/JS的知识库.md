...
- 声明了 `var b = 10;`，此时全局对象（如 window）上有 `b: 10`。

#### 2. 进入 IIFE（立即执行函数表达式）

- `(function b(){ ... })()` 这里的 `b` 是**函数表达式的名字**，只在函数体内部有效。
- 在函数作用域内部，**会创建一个只读的名为 `b` 的变量**，它指向这个函数本身。
- 这个内部的 `b` **屏蔽了外部的全局变量 `b`**。

#### 3. 作用域链结构

- 在函数体内，查找变量 `b` 时，**首先在当前函数作用域内找到函数名 `b`**，不会再往外层（全局）查找。
- 作用域链如下：
  - 当前函数作用域（有只读的 `b`，指向函数本身）
  - 全局作用域（有 `b = 10`）

#### 4. 赋值语句 `b = 20;`

- 这里其实是**尝试给函数作用域内的 `b` 赋值**。
- 但这个 `b` 是只读的（ES5 非严格模式下赋值无效，严格模式下会报错），所以赋值不会生效。

#### 5. `console.log(b);`

- 依然访问的是函数作用域内的 `b`，即函数本身。
- 所以输出 `function b(){ ... }`。
...
- **作用域链**决定了在函数内部访问 `b` 时，优先访问到的是函数作用域内的 `b`（即函数本身），而不是全局的 `b`。
- 这个内部的 `b` 是只读的，赋值无效。
- 所以 `console.log(b)` 输出的是函数本身。
